...
skip.if(developmentChains.includes(network.name)).
  describe('PriceExercise Integration Tests', async function () {
    ...
    beforeEach(async () => {
      const PriceConsumer = await deployments.get('PriceExercise')
      priceExercise = await ethers.getContractAt('PriceExercise', PriceConsumer.address)
    })
    ...
    it('Should successfully make an external API request andget a result', async () => {
      const transaction = await
      priceExercise.requestPriceData()
